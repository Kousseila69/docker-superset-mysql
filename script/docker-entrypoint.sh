...
# AUTHOR: Kousseila OUAGUENI <ouaguenikousseila@gmail.com>
# DESCRIPTION: apache superset docker container entrypoint file
# Modified/Revamped version of the https://github.com/apache/superset
...
help(){
  ...
  echo -e "## First download apache-superset docker image from docker-hub using command ${GREEN}'docker pull koussseila/superset-mysql:<tag>'${NC}                                               ##"
  ...
  echo -e "## Start ${BLUE}'server'${NC} container using command ${GREEN}'docker run -p 8088:8088 -v config:/home/superset/config/ kousseila/superset-mysql:<tag> cluster server <db_url> <redis_url>'${NC} ##"
  echo -e "## Start ${BLUE}'worker'${NC} container using command ${GREEN}'docker run -p 5555:5555 -v config:/home/superset/config/ kousseila/superset-mysql:<tag> cluster worker <db_url> <redis_url>'${NC} ##"
  echo -e "###########################################################################################################################################################################"
  echo
}
